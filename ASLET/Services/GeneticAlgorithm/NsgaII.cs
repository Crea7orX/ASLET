...
public class NsgaII<T> where T : Chromosome<T>
{
    ...
    public NsgaII(T prototype, int numberOfCrossoverPoints = 2, int mutationSize = 2, float crossoverProbability = 80,
        float mutationProbability = 3) : this(prototype, 100)
    {
    ...
    private Dictionary<int, float> CalculateCrowdingDistance(ISet<int> front, List<T> totalChromosome)
    {
        ...
        foreach (var key in front)
        {
            ...
            if (obj.Count > 1)
            {
                ...
                {
                    var diff = totalChromosome[sortedKeys[i + 1]].GetDifference(totalChromosome[sortedKeys[i - 1]]) *
                        1.0f / diff2;
